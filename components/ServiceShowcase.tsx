...
const OnderzoeksDienstenBanner: React.FC<OnderzoeksDienstenBannerProps> = ({ recommendations }) => {
  ...
  // Filteren: Alleen aanbevolen tonen tenzij 'showAll' aan staat
  const displayedServices = hasRecommendations && !showAll
    ? sortedServices.filter(s => recommendationMap[s.id])
    : sortedServices;
    ...
    <div className="w-full animate-fade-in">
      ...
      <div className="text-center mb-16">
        ...
        <p className="text-[#6B7280] mt-4 max-w-2xl mx-auto text-lg leading-relaxed font-light">
          {hasRecommendations
            ? "Op basis van uw casus zijn de volgende methoden het meest kosteneffectief en relevant."
              ...
              <div className="p-8 md:p-10 flex flex-col h-full">
                <div className="flex-grow flex flex-col items-center text-center px-2">
                  ...
                  <div className="flex justify-center mb-8">
                    <div className={`w-20 h-20 border rounded-2xl flex items-center justify-center transition-all duration-300 shadow-sm ${isRecommended
      ...
      {hasRecommendations && displayedServices.length < services.length && (
        <div className="mt-12 text-center">
          <button
            onClick={() => setShowAll(!showAll)}
            className="text-sm font-bold text-[#58B895] hover:text-[#13261f] transition-colors border-b border-[#58B895] border-dashed pb-1 uppercase tracking-widest"
          >
            {showAll ? "Toon alleen aanbevolen methoden" : "Bekijk alle onderzoeksmethoden"}
          </button>
        </div>
      )}
